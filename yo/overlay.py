...
class Overlay:
    ...
    def set_levels(self, rms_value: float, bands: list[float] | None = None, pcm=None) -> None:
        ...
        # Полная высота только на крике. Обычная речь — средняя.
        if self.rms < 0.0007:
            amp = 0.12
        else:
            amp = 0.14 + min(0.86, (self.rms - 0.0007) / 0.024 * 0.86)
...
def _draw_scene(cr, overlay: Overlay) -> None:
    ...
    _draw_status(cr, overlay, wave_x, status_y, wave_w, STATUS_HEIGHT)
...
def _voice_speed(rms_value: float, pcm) -> float:
    loud = min(1.0, max(0.0, (float(rms_value) - 0.0005) / 0.0045))
...
def _draw_mic_hint(cr, x, y, width, height, label: str = NO_MIC_HINT) -> None:
    ...
    while size >= 10.0:
        ...
        if tw <= max(8.0, width - 8.0):
            break
        size -= 1.0
    cr.set_source_rgba(0.86, 0.87, 0.89, 0.95)
    cr.move_to(x + (width - tw) / 2.0, y + height / 2.0 + th / 2.0)
    cr.show_text(label)
